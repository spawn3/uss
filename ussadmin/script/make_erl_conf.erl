...
main(Args) ->
    ...
    case lists:all(Pred, IpList) of
        true when length(IpList) > 0 ->
            ...
            DstDir = filename:join([?APPLICATION_ROOT, "data/conf"]),
            gen_master(filename:join([?APPLICATION_ROOT, "ussadmin/conf/master.config.tpl"]), DstDir, Options),
            gen_agent(filename:join([?APPLICATION_ROOT, "ussadmin/conf/agent.config.tpl"]), DstDir, Options),
...
get_plugins() ->
    case file:read_file(filename:join([?APPLICATION_ROOT, "ussadmin/conf/plugins.tpl"])) of
        {ok, B} ->
...
gen_agent(Path, DstDir, Options) ->
    {ok, [Terms]} = file:consult(Path),
    NewTerms = parse_conf(Terms, Options, []),
...
parse(kernel, [{managers, _OldValue}|T], Options, Acc) ->
    NewValue = proplists:get_value(managers, Options),
    parse(ussadmin, T, Options, [{managers, NewValue}|Acc]);
%
